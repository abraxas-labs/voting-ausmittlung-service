...
internal static class VoteInfoProportionalElectionMapping
{
    internal static IEnumerable<EventElectionResultDeliveryTypeElectionGroupResult> ToVoteInfoEchProportionalElectionGroups(
    ...
    private static CountingCircleResultType ToCountingCircleResult(ProportionalElectionResult result)
    {
        return new CountingCircleResultType
        {
            ...
            FullyCountedTrue = result.SubmissionDoneTimestamp.HasValue,
            ReleasedTimestamp = result.SubmissionDoneTimestamp,
            LockoutTimestamp = result.AuditedTentativelyTimestamp,
            VoterTurnout = result.CountOfVoters.VoterParticipation,
            CountingCircleId = result.CountingCircle.BasisCountingCircleId.ToString(),
            ...
            ElectionResult = new ElectionResultType
            {
                ...
                ProportionalElection = new ElectionResultTypeProportionalElection
                {
                    ...
                    CountOfEmptyVotesOfChangedBallotsWithoutListDesignation = (uint)result.TotalCountOfBlankRowsOnListsWithoutParty,
