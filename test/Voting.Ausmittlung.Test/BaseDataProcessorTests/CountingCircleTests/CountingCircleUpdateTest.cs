...
public class CountingCircleUpdateTest : CountingCircleProcessorBaseTest
{
    ...
    [Fact]
    public async Task TestEVotingCountingCircleShouldUpdateContestDetails()
    {
        ...
        var ccId = AusmittlungUuidV5.BuildCountingCircleSnapshot(contestId, CountingCircleMockedData.GuidStGallen);
        var contestDetail = await RunOnDb(db => db.ContestCountingCircleDetails
            .Where(x => x.ContestId == contestId && x.CountingCircleId == ccId)
            .SingleAsync());

        contestDetail.EVoting.Should().BeTrue();

        var contestIdBund = ContestMockedData.GuidBundesurnengang;
        var ccIdBund = AusmittlungUuidV5.BuildCountingCircleSnapshot(contestIdBund, CountingCircleMockedData.GuidStGallen);
        var contestDetailBund = await RunOnDb(db => db.ContestCountingCircleDetails
            .Where(x => x.ContestId == contestIdBund && x.CountingCircleId == ccIdBund)
            .SingleAsync());

        // for non e-voting contests, details should remain false
        contestDetailBund.EVoting.Should().BeFalse();
    }

    [Fact]
    public async Task TestNonEVotingCountingCircleShouldUpdateContestDetails()
    {
        await ContestMockedData.Seed(RunScoped);
        ...
        var contestId = ContestMockedData.GuidStGallenEvoting;
        var ccId = AusmittlungUuidV5.BuildCountingCircleSnapshot(contestId, CountingCircleMockedData.GuidUzwil);
        var contestDetail = await RunOnDb(db => db.ContestCountingCircleDetails
            .Where(x => x.ContestId == contestId && x.CountingCircleId == ccId)
            .SingleAsync());
