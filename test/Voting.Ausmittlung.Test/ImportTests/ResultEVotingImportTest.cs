...
public class ResultEVotingImportTest : BaseRestTest
{
    ...
    private static readonly ResultImportMeta _importMeta = new(
        ResultImportType.EVoting,
        Ech0222Version.V1,
    ...
    private readonly EVotingResultImportWriter _eVotingImportWriter;
    private bool _initializedAuthorizationTest;

    public ResultEVotingImportTest(TestApplicationFactory factory)
        : base(factory)
    {
        _eVotingImportWriter = GetService<EVotingResultImportWriter>();
    }
    ...
    [Fact]
    public async Task TestAsMonitoringElectionAdminV1ShouldWork()
    {
        ...
        using var resp = await MonitoringElectionAdminClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok));
    ...
    [Fact]
    public async Task TestAsMonitoringElectionAdminV3ShouldWork()
    {
        // set a state to submission finished
        await SetProportionalElectionResultState(ProportionalElectionResultMockedData.IdGossauElectionResultInContestStGallen, CountingCircleResultState.SubmissionDone);
        await SetProportionalElectionResultState(ProportionalElectionResultMockedData.IdUzwilElectionResultInContestUzwil, CountingCircleResultState.CorrectionDone);
    ...
    [Fact]
    public async Task TestShouldOnlyAccountForCountingCirclesThatHaveAPoliticalBusiness()
    {
        EventPublisherMock.Clear();

        // Change the contest to a higher DOI.
        // Everything should still work and only the counting circles that are actually part of a
        // political business should be accounted in e-voting imports.
        await TestEventPublisher.Publish(new ContestUpdated
        {
            Contest = new ContestEventData
            {
                Id = ContestMockedData.IdStGallenEvoting,
                DomainOfInfluenceId = DomainOfInfluenceMockedData.IdBund,
                Date = new DateTime(2020, 8, 31, 0, 0, 0, DateTimeKind.Utc).ToTimestamp(),
                Description = { LanguageUtil.MockAllLanguages("Kantonsurnengang") },
                EndOfTestingPhase = new DateTime(2018, 12, 20, 0, 0, 0, DateTimeKind.Utc).ToTimestamp(),
                EVoting = true,
                EVotingFrom = new DateTime(2020, 2, 20, 0, 0, 0, DateTimeKind.Utc).ToTimestamp(),
                EVotingTo = new DateTime(2020, 3, 1, 0, 0, 0, DateTimeKind.Utc).ToTimestamp(),
            },
        });

        using var client = base.CreateHttpClient(
            tenant: CountingCircleMockedData.Bund.ResponsibleAuthority.SecureConnectId,
            roles: RolesMockedData.MonitoringElectionAdmin);
        using var resp = await client.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok));
        resp.EnsureSuccessStatusCode();
    ...
    public async Task ContestLockedShouldThrow()
    {
        ...
        await AssertProblemDetails(
            () => MonitoringElectionAdminClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok)),
            HttpStatusCode.BadRequest,
    ...
    public Task OtherTenantShouldThrow()
    {
        var otherTenantClient = base.CreateHttpClient(
            ...
            roles: RolesMockedData.MonitoringElectionAdmin);
        return AssertStatus(
            () => otherTenantClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok)),
    ...
    public async Task CountingCircleResultAuditedAfterTestingPhaseEndedShouldThrow()
    {
        ...
        await AssertProblemDetails(
            () => MonitoringElectionAdminClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok)),
            HttpStatusCode.FailedDependency,
            $"A result is in an invalid state for an import to be possible ({ProportionalElectionResultMockedData.IdGossauElectionResultInContestStGallen})");
    }
    ...
    public async Task CountingCircleResultAuditedTentativelyInTestingPhaseShouldWork()
    {
        ...
        await AssertStatus(
            async () => await MonitoringElectionAdminClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok)),
            HttpStatusCode.OK);
    }
    ...
    public Task ContestNotFoundShouldThrow()
    {
        ...
        var importMeta = new ResultImportMeta(
            ...
            "test-eCH-0110.xml",
            Stream.Null);
        return Assert.ThrowsAsync<EntityNotFoundException>(() => _eVotingImportWriter.Import(req, importMeta));
    }
    ...
    public async Task UnknownCountingCirclesShouldBeIgnored()
    {
        ...
        import.AddEchMessageId("mock-eCH-0110-message-id");

        await _eVotingImportWriter.Import(import, _importMeta);
    ...
    public void DuplicateCountingCircleResultsShouldThrow()
    {
        ...
        var import = new VotingImport(
            ...
            Guid.Parse(ContestMockedData.IdUzwilEVoting))
        {
            VotingCards = NewValidVotingCardImportData(),
        };
    ...
    public async Task MajorityElectionShouldMapDuplicatesToInvalidIfInvalidAvailable()
    {
        ...
        var positions = new[]
        {
                VotingImportElectionBallotPosition.ForWriteIn("hans mueller"),
                VotingImportElectionBallotPosition.ForWriteIn("Hans Mueller"),
                VotingImportElectionBallotPosition.ForCandidateId(Guid.Parse(MajorityElectionMockedData.CandidateIdUzwilMajorityElectionInContestUzwil)),
                VotingImportElectionBallotPosition.ForCandidateId(Guid.Parse(MajorityElectionMockedData.CandidateIdUzwilMajorityElectionInContestUzwil)),
        };
        var req = NewValidMajorityElectionImportData([
            new VotingElectionBallot(
                    null,
                    false,
                    positions)
        ]);

        await _eVotingImportWriter.Import(req, _importMeta);
        ...
        await RunEvents<ResultImportCreated>(false);
        ...
        result.CountOfElectionsWithUnmappedEVotingWriteIns.Should().Be(0);
    ...
    public async Task MajorityElectionShouldMapDuplicatesToEmptyIfInvalidNotAvailable()
    {
        ...
        var positions = new[]
        {
                VotingImportElectionBallotPosition.ForWriteIn("hans mueller"),
                VotingImportElectionBallotPosition.ForWriteIn("Hans Mueller"),
                VotingImportElectionBallotPosition.ForWriteIn("hans müller"),
                VotingImportElectionBallotPosition.ForCandidateId(Guid.Parse(MajorityElectionMockedData.CandidateIdUzwilMajorityElectionInContestUzwil)),
        };
        var req = NewValidMajorityElectionImportData([
            new VotingElectionBallot(
    ...
    {
        TrySetFakeAuth();
        var ballotPositions = Enumerable.Repeat(VotingImportElectionBallotPosition.Empty, 10).ToList();
        var req = NewValidMajorityElectionImportData([
            new VotingElectionBallot(
                    null,
                    false,
        ...
        return AssertException<ValidationException>(
            () => _eVotingImportWriter.Import(req, _importMeta),
    ...
    public Task ProportionalElectionImportWithTooManyPositionsShouldThrow()
    {
        ...
        var result = req.PoliticalBusinessResults.OfType<VotingImportElectionResult>().First();
        var candidateId = Guid.Parse(ProportionalElectionMockedData.CandidateId11UzwilProportionalElectionInContestUzwil);
        var ballotPositions = Enumerable.Repeat(VotingImportElectionBallotPosition.ForCandidateId(candidateId), 10).ToList();
        result.Ballots =
        [
            new VotingElectionBallot(
                    null,
    ...
    public Task ProportionalElectionImportWithWriteInShouldThrow()
    {
        ...
        {
                VotingImportElectionBallotPosition.ForWriteIn("test"),
                VotingImportElectionBallotPosition.Empty,
                VotingImportElectionBallotPosition.Empty,
        };
        result.Ballots =
        [
    ...
    public Task ProportionalElectionImportWithUnmodifiedUnknownListShouldThrow()
    {
        TrySetFakeAuth();
        var req = NewValidProportionalElectionImportData();
        var ballot = req.PoliticalBusinessResults.OfType<VotingImportElectionResult>().First().Ballots.First();
        ballot.Unmodified = true;
        ballot.ListId = Guid.Parse("4869ef1b-0497-4e24-8e01-a03013ed79a2");
        return Assert.ThrowsAsync<EntityNotFoundException>(() => _eVotingImportWriter.Import(req, _importMeta));
    }
    ...
    public Task ProportionalElectionImportWithTripledCandidateShouldThrow()
    {
        ...
        var ballotPositions = new[]
        {
                VotingImportElectionBallotPosition.ForCandidateId(candidateId),
                VotingImportElectionBallotPosition.ForCandidateId(candidateId),
                VotingImportElectionBallotPosition.ForCandidateId(candidateId),
        };
        var req = NewValidProportionalElectionImportData();
    ...
    public Task VoteImportWithUnknownQuestionNumberThrow()
    {
        ...
        var result = req.PoliticalBusinessResults.OfType<VotingImportVoteResult>().First();
        var ballotResult = result.BallotResults.First();

        var questionAnswers = new[]
        {
                new VotingImportVoteBallotQuestionAnswer(3, BallotQuestionAnswer.Yes),
        };

        ballotResult.Ballots =
        [
            new VotingVoteBallot(questionAnswers, Array.Empty<VotingImportVoteBallotTieBreakQuestionAnswer>())
        ];
    ...
    public Task VoteImportWithUnknownTieBreakQuestionNumberThrow()
    {
        ...
        {
                new VotingImportVoteBallotTieBreakQuestionAnswer(2, TieBreakQuestionAnswer.Q1),
        };

        ballotResult.Ballots =
        [
            new VotingVoteBallot(Array.Empty<VotingImportVoteBallotQuestionAnswer>(), tieBreakQuestionAnswers)
        ];
    ...
    [Fact]
    public void DuplicateCountOfVotersInformationShouldThrow()
    {
        TrySetFakeAuth();
        var ballotPositions = Enumerable.Repeat(VotingImportElectionBallotPosition.Empty, 10).ToList();
        var req = NewValidMajorityElectionImportData([
        ...
        var id = Guid.Parse(MajorityElectionMockedData.IdUzwilMajorityElectionInContestUzwilWithoutChilds);
        AssertException<ValidationException>(
            () => req.SetTotalCountOfVoters(
                id,
                CountingCircleMockedData.IdUzwil,
                123),
    ...
    protected override async Task<HttpResponseMessage> AuthorizationTestCall(HttpClient httpClient)
    {
        ...
        return await httpClient.PostFiles(BuildUri(), ("ech0222File", TestFileV1Ok), ("ech0110File", TestFileEch0110Ok));
    }
    ...
    private Uri BuildUri(Guid? contestId = null)
        => new($"api/result_import/e-voting/{contestId?.ToString() ?? ContestMockedData.IdStGallenEvoting}", UriKind.RelativeOrAbsolute);

    private VotingImport NewValidProportionalElectionImportData(Guid? contestId = null)
    {
        var ballots = new[]
        {
            new VotingElectionBallot(
        ...
        var results = new List<VotingImportPoliticalBusinessResult>()
        {
            new VotingImportElectionResult(
                Guid.Parse(ProportionalElectionMockedData.IdUzwilProportionalElectionInContestUzwilWithoutChilds),
                CountingCircleMockedData.IdUzwil,
        ...
        results[0].TotalCountOfVoters = 123;
        ...
        import.AddResults(results);
        return import;
    }

    private VotingImport NewValidMajorityElectionImportData(VotingElectionBallot[]? ballots = null)
    {
        ballots ??=
        [
            new VotingElectionBallot(
    ...
    private VotingImport NewValidVoteImportData()
    {
        var ballots = new[]
        {
            new VotingVoteBallot(
        ...
        {
            new VotingImportVoteBallotResult(
        ...
        var results = new List<VotingImportPoliticalBusinessResult>()
        {
            new VotingImportVoteResult(
                Guid.Parse(VoteMockedData.IdUzwilVoteInContestUzwilWithoutChilds),
                CountingCircleMockedData.IdUzwil,
                ballotResults),
        };

        var imports = new VotingImport(
            "my-mock-ech-message-id",
            Guid.Parse(ContestMockedData.IdUzwilEVoting))
        {
            VotingCards = NewValidVotingCardImportData(),
        };
        imports.AddResults(results);
        return imports;
    }

    private List<VotingImportCountingCircleVotingCards> NewValidVotingCardImportData()
    {
        return
        [
            new(CountingCircleMockedData.IdUzwil, 123)
